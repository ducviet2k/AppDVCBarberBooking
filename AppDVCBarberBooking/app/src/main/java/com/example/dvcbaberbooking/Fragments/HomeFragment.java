                ...
                public void onSuccess(Void aVoid) {
                    ...
                    if (Paper.book().read(Common.EVENT_URI_CACHE) != null) {
                        ...
                        if (eventString != null && TextUtils.isEmpty(eventString))
                            uri = Uri.parse(eventString);
                        if (uri != null)
                            getActivity().getContentResolver().delete(uri, null, null);
                    }
                    Toast.makeText(getActivity(), "Bạn đã huỷ lịch !", Toast.LENGTH_SHORT).show();
//                    Toast.makeText(getActivity(), "Succsess delete Booking", Toast.LENGTH_SHORT).show();
    ...
    com.google.firebase.firestore.EventListener<QuerySnapshot> userBookningEvent = null;
...
//    private String mParam2;
    ...
    private void loadUserBooking() {
        ...
        //part25
        if (userBookningEvent != null) {
            if (userBookingListener == null) {
                userBookingListener = userBooking
                        .addSnapshotListener(userBookningEvent);
            }
...
//        }
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        if (user != null) {
            ...
            loadBanner();
            loadLookBook();
            initRealtimeUserBooking(); //load userbooking
            loadUserBooking();
            countCartItem();
        }
    ...
    private void initRealtimeUserBooking() {
        if (userBookningEvent == null) {
            userBookningEvent = new EventListener<QuerySnapshot>() {
                @Override
